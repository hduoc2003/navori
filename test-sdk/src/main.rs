...
async fn main() -> anyhow::Result<()> {
    ...
    println!("total time: {}", verify_merkle_stat.time + verify_fri_stat.time + rcmp_stat.time + vpar_stat.time);
    println!("total gas used: {}", verify_merkle_stat.gas_used + verify_fri_stat.gas_used + rcmp_stat.gas_used + vpar_stat.gas_used);
    Ok(())
}
